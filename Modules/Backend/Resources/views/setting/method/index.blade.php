...
<div class="kt-content  kt-grid__item kt-grid__item--fluid" id="kt_content">
                    ...
                    <form method="POST" id="form-filter" class="m-form m-form--fit m--margin-bottom-20">
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                <label>Name</label>
                                <input type="text" class="form-control" name="method_name" id="method_name" placeholder="Enter method name" />
                            </div>
                            <div class="col-md-4 mb-3">
                                <label>Module</label>
                                <select  class="form-control selectpicker" name="module_id" id="module_id" data-live-search="true" data-live-search-placeholder="Search" title="Select Please"></select>
                            </div>
                            <div class="col-md-4 mb-3">
                                <div class="mt-25px">    
    ...
    <!--Begin:: Modal-->
    @include('backend::setting.method.modal')
    <!--End:: Modal-->
</div>
@endsection
...
$(document).ready(function () {
    ...
    table = $('#dataTable').DataTable({
        ...
        "ajax": {
            "url": "{{route('admin.method.list')}}",
            "type": "POST",
            "data": function (data) {
                data.method_name  = $('#form-filter #method_name').val();
    ...
    $(document).on('click', '#save-btn',function(event){
        let url = "{{route('admin.method.store')}}";
    ...
    $(document).on('click','.edit_data',function () {
        ...
        $.ajax({
            url: "{{route('admin.method.edit')}}",
            type: "POST",
            data:{id:id,_token:_token},
            dataType: "JSON",
            success: function (data) {
                $('#saveDataForm #update_id').val(data.method.id);
                $('#saveDataForm #method_name').val(data.method.method_name);
                $('#saveDataForm #method_slug').val(data.method.method_slug);
                $('#saveDataForm #module_id').val(data.method.module_id);
                $('#saveDataForm .selectpicker').selectpicker('refresh');
                $('#saveDataModal').modal({
                    keyboard: false,
                    backdrop: 'static', //make modal static
                });
                $('.modal-title').html('<i class="fas fa-edit"></i> <span>Edit '+data.method.method_name+' Data</span>');
    ...
    $(document).on('click','.delete_data',function () {
        ...
        let id  = $(this).data('id');
        let url = "{{route('admin.method.delete')}}";
    ...
    $(document).on("click",'#bulk_action_delete',function() {
        ...
        if(id.length === 0) //tell us if the array is empty
        {
            ...
            let url = "{{route('admin.method.bulkaction')}}";
...
function get_module_list(){
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data) {
                $('#form-filter #module_id').html(data);
                $('#saveDataForm #module_id').html(data);
                $('.selectpicker').selectpicker('refresh');
            } 
        },
        error: function (xhr, ajaxOptions, thrownError) {
