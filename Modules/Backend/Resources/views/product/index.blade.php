...
<div class="kt-subheader   kt-grid__item" id="kt_subheader">
    ...
    <div class="kt-subheader__toolbar">
        <div class="kt-subheader__wrapper">
            <button type="button" onclick="show_modal(modal_title='Add New Product', btn_text='Save')" class="btn btn-brand btn-icon-sm btn-sm">
...
<div class="kt-content  kt-grid__item kt-grid__item--fluid" id="kt_content">
                    ...
                    <form method="POST" id="form-filter" class="m-form m-form--fit m--margin-bottom-20">
                        <div class="row">
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <select  class="form-control selectpicker" name="category_id" id="category_id" data-live-search="true" data-live-search-placeholder="Search" title="Choose one of the following...">
                                    <option value="">Select please</option>
    ...
    @include('backend::product.show-modal')
    <!--End:: Modal-->
</div>
@endsection
...
$(document).ready(function () {
    ...
    $(document).on('click','.view_data',function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                $('#showDataModal .modal-body').html(data.product);
                $('#showDataModal').modal({
                    keyboard: false,
                    backdrop: 'static', //make modal static
                });
                $('.modal-title').html('<i class="fas fa-eye"></i> <span>'+data.product_name+' Data</span>');
            },
    ...
    $(document).on('click','.edit_data',function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#saveDataForm #product_name').val(data.product.product_name);
                $('#saveDataForm select[name="brand_id"]').val(data.product.brand_id);
                $('#saveDataForm select[name="category_id"]').val(data.product.category_id);
                $('#saveDataForm select[name="status"]').val(data.product.status);
                $('#saveDataForm .selectpicker').selectpicker('refresh');
                $('#saveDataForm #description').val(data.product.description);
                let file = "{{'storage/'.PRODUCT}}"+data.product.product_image;
                $('#saveDataForm #product-image').attr('src',file);
                $('#saveDataForm #old_product_image').val(data.product.product_image);
                $('#saveDataModal').modal({
                    keyboard: false,
                    backdrop: 'static', //make modal static
                });
                $('.modal-title').html('<i class="fas fa-edit"></i> <span>Edit '+data.product.product_name+' Data</span>');
    ...
    $(document).on('click','.delete_data',function () {
        ...
        let id  = $(this).data('id');
        let url = "{{route('admin.product.delete')}}";
    ...
    $(document).on("click",'#bulk_action_delete',function() {
        ...
        if(id.length === 0) //tell us if the array is empty
        {
            ...
            let url = "{{route('admin.product.bulkaction')}}";
