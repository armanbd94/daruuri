...
<div class="kt-subheader   kt-grid__item" id="kt_subheader">
    ...
    <div class="kt-subheader__toolbar">
        <div class="kt-subheader__wrapper">
            @if (permission('highlighted-service-add'))
            <button type="button" onclick="show_modal(modal_title='Add New Highlighted Service', btn_text='Save')" class="btn btn-brand btn-icon-sm btn-sm">
...
<div class="kt-content  kt-grid__item kt-grid__item--fluid" id="kt_content">
    ...
    <!--Begin:: Modal-->
    @include('backend::highlighted-service.modal')
    <!--End:: Modal-->
</div>
@endsection
...
$(document).ready(function () {
    ...
    table = $('#dataTable').DataTable({
        ...
        "ajax": {
            "url": "{{route('admin.highlighted.service.list')}}",
            "type": "POST",
            "data": function (data) {
                data.service_name  = $('#form-filter #service_name').val();
                data._token = _token;
            }
        },
        ...
        @if (permission('highlighted-service-report'))
        "dom": 'lTgBfrtip',
        "buttons": [
            'colvis',
            {
                ...
                "filename": 'highlighted-service-report',
                "exportOptions": {
                     columns: ':visible'
                }
            },
    ...
    $('#saveDataForm').on('submit', function(event){
        ...
        let url = "{{route('admin.highlighted.service.store')}}";
        let method;
        if(id){
            method = "update";
        }else{
    ...
    $(document).on('click','.edit_data',function () {
        ...
        $.ajax({
            url: "{{route('admin.highlighted.service.edit')}}",
            type: "POST",
            data:{id:id,_token:_token},
            dataType: "JSON",
            success: function (data) {
                $('#saveDataForm #update_id').val(data.service.id);
                $('#saveDataForm #service_name').val(data.service.service_name);
                $('#saveDataForm #description').val(data.service.description);
                $('#saveDataForm #sorting').val(data.service.sorting);
                let file = "{{'storage/'.SERVICE}}"+data.service.image;
                $('#saveDataForm #service-image').attr('src',file);
                $('#saveDataForm #old_image').val(data.service.image);
                $('#saveDataModal').modal({
                    keyboard: false,
                    backdrop: 'static', //make modal static
                });
                $('.modal-title').html('<i class="fas fa-edit"></i> <span>Edit '+data.service.service_name+' Data</span>');
    ...
    $(document).on('click','.delete_data',function () {
        ...
        let id  = $(this).data('id');
        let url = "{{route('admin.highlighted.service.delete')}}";
    ...
    $(document).on("click",'#bulk_action_delete',function() {
        ...
        if(id.length === 0) //tell us if the array is empty
        {
            ...
            let url = "{{route('admin.highlighted.service.bulkaction')}}";
